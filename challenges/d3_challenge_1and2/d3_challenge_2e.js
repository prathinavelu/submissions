...
d3.csv('d3_mta_data.csv', function(raw_data) {
    ...
    var data = d3.nest()
    	     ...
    	     .rollup(function(leaves) { return {"total_rides": d3.sum(leaves, function(d) {return parseFloat(d.ridership);})} })
    	     .entries(raw_data);
    console.log('test',data)
    
    var boroughs = data.map(function(b) {return b.key;});   
    ...
    var bmax = data.map(function(b) {return d3.max(b.values, function(s) {return +s.values.total_rides;});})
    var allmax = d3.max(bmax, function(d) {return d})
	...
	.attr('transform',function(d){return 'translate(' + xB(d.key) + ',0)';})
	...
	.data(function(d){console.log('stationvalues',d.values);return d.values})
	.enter().append('g')
    ...
    // Use xS.rangeBand() for the width.
      ...
      .append('rect')
      .attr('x',0)
      .attr('y', function(d) { return y(d.values.total_rides)})
      .attr('width',xS.rangeBand())
      .attr('height',function(d) { return h - y(d.values.total_rides)})
      .attr('fill','gray')
